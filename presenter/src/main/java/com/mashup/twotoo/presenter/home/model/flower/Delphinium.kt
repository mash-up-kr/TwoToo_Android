 ...
 */
data class Delphinium(val userType: UserType, val growType: Stage) : FlowerType(
    name = FlowerName.Delphinium,
) {
    override val flowerImage: FlowerImage = when (userType) {
        ME -> {
            when (growType) {
                ...
                Fourth -> FlowerImage(image = R.drawable.img_home_fourth_stage_delphinium_me, width = 127.dp, height = 211.dp)
                Fifth -> FlowerImage(image = R.drawable.img_home_fifth_stage_delphinium_me, width = 127.dp, height = 211.dp)
            }
        }
        PARTNER -> {
            when (growType) {
                ...
                Third -> FlowerImage(image = R.drawable.img_home_third_stage_me, width = 102.dp, height = 179.dp)
                Fourth -> FlowerImage(image = R.drawable.img_home_fourth_stage_delphinium_partner, width = 127.dp, height = 211.dp)
                Fifth -> FlowerImage(image = R.drawable.img_home_fifth_stage_delphinium_partner, width = 127.dp, height = 211.dp)
            }
        }
    }
}
